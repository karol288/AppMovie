...
@Component({
  ...
  styleUrls: ['./buscador-pelicula.component.css']
})
export class BuscadorPeliculaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(this.texto); // Verifica el texto ingresado
  }

  //esta funcion va a ser la encargada de buscar la pelicula por el texto
  buscarPeliculas() {
    this.service.getMovie(this.texto).subscribe((res: any) => { //decir que res va ser de tipo any es porque el va a recibir cualquier tipo de dato
      ...
      console.log(this.noPelicula); // Muestra en consola el mensaje de error o vacío
    });
  }
